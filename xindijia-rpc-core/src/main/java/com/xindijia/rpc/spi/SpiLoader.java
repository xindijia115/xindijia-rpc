...
/**
 * @author xia
 * @since 2024/3/16/15:00
 * 读取配置并加载实现类的方法,SPI加载器(支持键值对映射)
 */
@Slf4j
public class SpiLoader {
    ...
    public static <T> T getInstance(Class<?> tClass, String key) {
        ...
        if (keyClassMap == null) {
            throw new RuntimeException(String.format("SpiLoader未加载 %s 类型", name));
        }
        if (!keyClassMap.containsKey(key)) {
            throw new RuntimeException(String.format("SpiLoader的 %s 不存在key=%s的类型", name, key));
        }
        //获取要加载的实现类型
